...
async def upload_map(file: UploadFile = File(...)):
    ...
    # You can also get width and height from image.size if needed.
    tiles = []
    tile_id = 1
    for y in range(image.height):
        ...
        for x in range(image.width):
            ...
            # Use the same mapping as in image_tile_generator
            tile_type = {
                (144, 238, 144): "plains",
                (34, 139, 34): "forest",
...
def create_new_player(player_id: str, name: str = "Player"):
    if not create_player(player_id, name):
        raise HTTPException(status_code=400, detail="Player already exists.")
    return {"message": f"Player {player_id} created."}
